...
2-1 데이터 입력은 string 차원의 텍스트, 그리고 파일로 image와 video를 업로드 한다.

2-2 업로드된 이미지와 Video는 DB에 저장하지 않고 목록만 가지고 있는 형태다.

2-3 채팅 창을 통해서는 모든 대화가 Text만으로 이루어진다. 즉 이 창에서는 이미지나 비디오가 보여지지 않는다.
